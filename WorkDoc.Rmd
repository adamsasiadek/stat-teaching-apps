...
# Set echo=FALSE in a code chunk it its interactive content has been added.
knitr::opts_chunk$set(fig.width=12, echo=TRUE, warning=FALSE, message=FALSE, eval=TRUE, tidy=TRUE)
```

```{r simulation_settings, eval=FALSE, echo=FALSE}
# SPSS data set candy colour (uniform) and weight (normal, M = 2.8, SD = 0.2), N= 50.
set.seed(seed = 12345)
candies <- data.frame(colour = factor(c("blue","yellow","green","red","black")), weight = rnorm(50, mean = 2.8, sd = 0.2), sticky = factor(c(rep(c("sticky", "sticky", "not sticky", "sticky", "not sticky"), 6), rep(c("sticky", "not sticky"), 9), c("not sticky", "sticky"))))
...
```{r sampling_distribution_summary, fig.cap="Sampling distribution summary"}
...
\@ref(fig:sampling_distribution_summary) simulates drawing random samples from a candy factory's stock of candies. The histogram on the Discrete tab shows the distribution of candies according to colour. Draw some samples and have a look at the number of yellow candies in each sample as well as the average number of yellow candies over all samples.
...
\@ref(fig:random_variable) shows a population of candies. The button allows you to draw a random sample of ten candies from this population. Before you press the button, answer the question: What do you expect the number of yellow candies in the sample to be? Then, draw several samples and check whether your expectations come true.

```{r random_variable}
#interactive content: draw a sample from a population of points uniformly distributed over five colors, display the sample (as a set of colored circles) and the number of yellow candies with each sample ; relevant expectations: (1) number of yellow candies per sample varies (variable), (2) this number depends on chance (random variable), (3) the number may range from 0 to 10 (sampling space), (4) the most likely number of yellow candies is two (expected value, expectation). User can set the population proportion of yellow candies (population proportions of other candy colors are automatically adjusted with random variation).
```
...
The collection of all possible outcome scores of a random variable is called _sampling space_. In the example of a (sample) bag containing ten candies, the number of yellow candies can range from zero (I am very very unlucky) to ten (I am super lucky).  The sampling space then contains the integer values from zero to ten.
...
\@ref(fig:random_variable) displays the population of candies (candy factory's stock, top), a random sample from this population (one bag, middle), and the sampling distribution (bottom). Draw one random sample.

1. What are the units of observation in the three histograms? Hint: There are two different types of units.  
2. What do the numbers on the horizontal axis of the histograms mean?  
...
#Generate a binomial probability distribution for the number of yellow candies in a random sample of ten from a population with the specified proportion of yellow candies and display this as a table ; the user is able to change the population proportion, which is initially set to .2.
```

\@ref(fig:probability_distribution) displays the probability distribution of the number of yellow candies per bag of twenty candies. This is an example of a _discrete probability distribution_ because only a limited number of outcomes are possible, so it is feasible to list the probability of each outcome separately.
...
Use \@ref(fig:expected_value_exercise) to practice our understanding of expected values. When you enter your expected value, the sampling distribution and its mean are shown.

```{r expected_value_exercise}
#Several scenarios (choice list) for discrete (symmetric?) probability distributions (dice, coin flips), entering a number in the app displays the probability distribution as a histogram with the mean (expected value.
# Examples: number of heads per toss if we toss 2, 3, 4 unbiased coins; average of the number of eyes per throw if we throw 1, 2 unbiased dice; sum of the number of eyes per throw if we throw 1, 2, 3 unbiased dice
```
...
The sample mean is an unbiased estimator of the population mean, so the average weight of all candies in the population (at the factory) is the average of the (candy weights in the) sampling distribution. And this is the average weight that we expect in a sample drawn from this population (the expected value or expectation). So far, everything is the same as in the case of the proportion of yellow candies, which was a discrete random variable because it could take only a limited et of values: yellow, blue, green, red, and orange.  

#### Continuous probabilities  

```{r continuous_probabilities}
...
\@ref(fig:continuous_probabilities) shows a typical probability distribution of average candy weight.
...
If we cannot determine the probability of a single value and we have to link probabilities to a range af values (average candy weight above/below 2.8 gram), how can we display probabilities? We have to display a probability as an area between the horizontal axis and a curve. This curve is called a _probability density function_, so if there is a label to the vertical axis of a continuous probability distribution, it usually is "Probability density" instead of "Probability".

\@ref(fig:p_values) shows an example of a continuous probability distribution for the average weight of candies in a sample bag. This is the familiar normal distribution so we could say that the normal function is the probablity density function here. The total area under this curve is set to one, so the area belonging to a range of sample outcomes (average candy weight) is 1 or less, as probabilities should be.

```{r p_values}
# Generate a normal sampling distribution representing average candy weight in a sample bag ; add two slider(s) to the x-axis as thresholds, showing the proportions of the probabilities to left and to right and between
```

Practice understanding and reading probabilities in a continuous sampling distribution with \@ref(fig:p_values). 
...
Are you struggling with the idea of areas instead of heights (values on the vertical axis) as probabilities? Just realize that we may use the area of a bar in a histogram instead of the height as indication of the probability in \@ref(fig:continuous_probabilities). After all, the bars in a histogram are all equally wide, so differences between bar areas are proportional to differences in bar height. 

#### Probabilities always sum to 1
While you were playing with \@ref(fig:p_values), you may have noticed that displayed probabilities always add up to one. This is true for every probability distribution as you have learned before. In addition, you may have realized that the probabilities can also be interpreted as proportions or percentages. The probability that a sample bag contains candies with average weight over 2.9 gram is equal to the proportion of samples in the sampling distribution with average candy weight over 2.9 gram. Thus, we can use probabilities to find the threshold values that separate the top ten percent or the bottom five percent in a distribution.  
...
# a button to generate a population with two variables (exposure and awareness of risks) with a randomly generated small positive or negative slope and one small random sample from the population with a regression line and regression equation depicted ; a select list with options to choose from ('positive/increasing', 'negative/decreasing', 'horizontal/no relation') ; on selecting a choice, the slope regression equation for the generated population is shown with a remark about the student's choice ('Correct'/'Not correct')
# goal: to sensitize the student to the risks of fully trusting the sample result
```

\@ref(fig:regression_example) shows a random sample with the regression line and equation predicting risk awareness from campaign exposure.

1. What do you think is the slope of the regression line in the population (select one option?  
...
```{r 3_means}
#Generate a population and sample distribution of candy weight, and sampling distribution of average candy weight. Add the average of each distribution as a vertical line. Both the vertical lines and the x-axis of the sampling distribution should be sensitive to clicks. The mean of the sample distribution and x-axis of the sampling distribution represent sample means.
```

\@ref(fig:3_means) contains a population, sample, and sampling distribution.  

1. Click on the elements that represent sample means.  
2. Which means do not represent the average weight of candies in a sample? What do these averages mean?  
3. Is it accidental that the mean of the population and sampling distribution coincide?  

The sampling distribution, here, is a distribution of sample means but the sampling distribution itself also has a mean, which is called the expected value or expectation of the sampling distribution. Don't get confused! The mean of the sampling distribution is the average of the average weight of candies across all possible samples. This mean of means has the same value as our third mean, namely the average weight of the candies in our sample becausea sample mean is an nbiased estimator of the population mean.  
...
```{r models_summary}
# Interface with three tabs labelled bootsrap/exact/theoretical, containing, (1) app bootstrapping, (2) app exact-approach, (3) app normal_approximation.
```
...
In \@ref(fig:bootstrapping), draw bootstrap samples to build a sampling distribution of the proportion of yellow candies in a sample. The five colours are equally distributed in the population.

1. What is the true sampling distribution in this example? If in doubt, see \@ref(fig:probability_distribution).  
2. Does the bootstrap sampling distribution resemble the true sampling distribution? Justify your answer.  
...
In \@ref(fig:replacement), draw samples with replacement and some samples without replacement.
...
If we do allow the same person to be sampled more than once, we sample _with replacement_. The same person can occur more than once in a sample. Bootstrap samples that are sampled with replacement from the original sample can vary because they need not contain all observations in the original sample. Some observations may not be sampled while other observations are sampled several times. You probably have noticed this in \@ref(fig:replacement). Sampling with replacement, we can obtain different bootstrap samples from the the original sample and still have bootstrap samples of the same size as the original sample.  
...
```{r bootstrap_limitations}
# Draw a sample of the user-specified size from a uniformly distributed population (five candy colors) ; show the sample (as dot plot) and collect the proportion of yellow candies for 1,000 bootstrap samples into a histogram, which already shows the true sampling distribution (binomial distribution) as a light histogram in the background
```

In \@ref(fig:bootstrap_restrictions), you can adjust the size of the sample drawn from a population of candies equally distributed over five colours. The light histogram in the background represents the true sampling distribution.
...
![Screen shot of SPSS bootstrap dialog](figures/SPSSbootstrap.png)
...
The confidence level of the confidence interval can be changed. We will discuss confidence levels in Chapter \@ref(@@). For now, never mind this option because the default (95%) is most widely used. There are two ways of calculating the confidence interval from the bootstraps. _Bias corrected accelerated (BCa)_ is the better option. Finally, it is possible to stratify the bootstrap sample. If you select a variable here, for example, candy colour, each bootstrap sample with have the same distributionof colours as the original sample. Usually, this is not necessary.
...
<iframe width="100%" height="925" src="bootstrap.htm" frameborder="0" allowfullscreen></iframe>

The first table in the output merely tells us the options we selected for bootstrapping. The table __Group Statistics__ gives us the group means, standard deviations, and standard error of the mean (to be discussed in Chapter \@ref(@)). Each of these has a bootstrap confidence interval, illustrating that we can bootstrap any sample statistic. 
...
```{r exact-approach}
#See app: expected_value_exercise. Several scenarios (choice list or randomly generated?) for discrete (symmetric?) probability distributions (dice, coin flips) ; display outcomes, calculation (graphical: each combination for this outcome, e.g.,  0 = heads/heads = 0.25, 1 = heads/tails + tails/heads = 0.50, 2 = heads/heads = 0.25), and calculated probabilities.
# Examples: number of heads per toss if we toss 2, 3 unbiased coins; average of the number of eyes per throw if we throw 1, 2 unbiased dice; sum of the number of eyes per throw if we throw 1, 2 unbiased dice
```
...
If SPSS offers an exact approach of the sampling distribution, the test dialog window contains an _Exact_ button. You will find it in the dialog for crosstabs ( _Analyze>Descriptive Statistics>Crosstabs_ ) and in several legacy dialogs for non-parametric tests ( _Analyze>Nonparametric Tests>Legacy Dialogs_ ). 
...
For the test result, you should interpret the p value reported in the table __Chi-Square Tests__. This value is clearly below .05 so we conclude that the test is statistically significant. It is unlikely that all colours are equally sticky in the population (_p_ = .010). According to the percentages in the cross-tabulation, blue, red, and yellow candies are more often sticky than black and green candies and the association is strong (Phi = .52). 

In Section 2.3 \@ref(@), we will learn that we should use the exact p value instead of the default asymptotic p value in a table such as the current one because of the low expected cell values.
...
```{r normal_approximation}
# See app: p_values.
# Let a button generate a normal sampling distribution (with a random SD) representing average candy weight in a sample bag ; represent it as a histogram (with fixed x-axis, so distributions with different SD have different widths) and project the normal function on top of it ; colour the bars for the upper and lower 2.5% of cases in the histogram (to draw attention to the tails as important areas) ; add two vertical lines to the graph demarcating the outer 2.5% of the area under the normal curve (display the probabilities to left and to right)
```

1. Make the computer draw a lot of samples to generate a sampling distribution of the sample means. Check if the normal function is a good approximation of the sampling distribution. Pay special attention to the tails because these are used for significance tests (see Chapter 4 \@ref(@)). Repeat this a few times.

The normal distribution is a mathematical function linking continuous scores, e.g., a sample statistic such as the average weight in the sample, to p-values, that is, to the probability of finding at least or at most this score. Such a function is called a _probability density function_, Chapter 1 \@ref(@).  
...
\@ref(fig:normal_approx_proportion) shows an example in which the normal distribution is a good approximation for the sampling distribution of a proportion in some situations but not in all situations.

```{r normal_approx_proportion}
...
For most of these theoretical probability distributions, sample size is important. The larger the sample, the better. There are additional conditions that must be satisfied such as the distribution of the variable in the population. The rules of thumb are summarized in \@ref(@).  

Table @. Rules of thumb for the use of theoretical probability distributions.

Distribution  | Sample statistic | Minimum sample size | Other requirements
:-------------------| :---------- | :------------------: | :-----------------
Normal distribution | proportion | >= 5 divided by test proportion (<= .5)  | 
Normal distribution | one or two means | > 100 | OR variable is normally distributed in the population and population standard deviation is known
_t_ distribution | one or two means | > 30  | OR variable is normally distributed in each group's population
_t_ distribution | (Pearson) correlation coefficient | -  | variables are normally distributed in the population
_t_ distribution | (Spearman) rank correlation coefficient | > 30  | -
_t_ distribution | regression coefficient | -  | - 
_F_ distribution | 3+ means  |  all groups are more or less of equal size  | OR all groups have the same population variance
_F_ distribution | two variances | -  | no conditions for Levene's _F_ test
chi-squared distribution | row or cell frequencies | expected frequency >= 1 and 80% >= 5 | crosstab: 3+ rows or 3+ columns 

* Note: Bootstrapping and exact tests can be used if conditions for theoretical probability distributions have not been met. Special conditions apply to regression analysis (see mediation and moderation).  

#### Checking conditions  

```{r check_conditions}
# "Let me try" button generates a sample that does or does not meet conditions for using a theoretical probability distribution and ask which theoretical distribution is relevant (list: normal/t/F/chi-squared) and whether conditions are met (Yes/No choice). Give feedback (correct answer) to student's choice.
# Specify null hypothesis (sometimes includes test value), sample size, and sample distribution (with expected values for chi-squared tests). 
```
...
We can only use the data in our sample to make an educated guess about the distribution of the variable in the population. For example, if the scores in our sample are clearly normally distributed, it is plausible that the scores in the population are normally distributed. We do not know that the population distribution is normal but we assume it is. If the sample distribution is clearly not normally distributed, we do not assume that the population is normally distributed. In short, we smotimes have to make assumptions when we decide on using a theoretical probability distribution.
...
```{r mean_dependent}
# Demonstrate the construction of a sampling distribution for mean differences for paired/dependent samples; two (very small) populations (normal distributions) represented as dotplots with columns of dots shaded from dark yellow to pale yellow (with their means?), the second population (repeated measurement) is more pale on average ; (button or rather) clicking a dot in the dotplot of the first population shows its ID number and the dot with the same ID in the second population (note that ID can be random number and selected dot in second populaiton can also be random as long as its color is paler) ; show the difference score for the selected dots and store it as value in the sample ; show the current sample mean ; if a sample of five (difference scores) has been created, store the sample mean in the smapling distribution (bottom histogram); add button to add/animate many more samples, with results stored in the sampling distribution
```

1. Select an observation (dot) in the chart at the top-left ("Population at time 1") and repeat this five times. Why do you have to select one observation at a time instead of selecting the entire sample from the first population and then a sample from the second population?  
2. Which is more important: the difference between the means of the two populations (top) or the mean of the differences in the sample (middle and bottom)?  
...
As a consequence, probabilities have to be calculated in a different way, so we need a special sampling distribution. In the interactive content above, you may have noticed a relatively simple solution for two repeated measurements. We just calculate the difference between the two measurements for each candy in the sample and use the mean of this new difference variable as the sample statistic that we are interested in. In contrast, we calculated the differences at the level of the sample (candy bag) in the situation with two independent samples.  

For this sample statistic, we construct a sampling distribution. The _t_-distribution, again, offers a good approximation of this sampling distribution if the sample is not too small. The actual sampling distribution can become quite complicated but we need not worry about that. If we choose the right technique, our statistical software will take care of this. Of course, we should check whether the conditions are met for approximating the sampling distribution with a theoretical probability distribution.  

Repeated measurements on the same set of sampling units, for example, candies or respondents, are called dependent samples because the sample for the repeated measurement cannot be drawn wihtout taking into account the first sample. If we have drawn the sample for our first measurement, the sample for our second measurement is fixed because we are going to use the same candies or respondents. 

In contrast, samples are independent if we can draw each sample without taking into account the other samples. In the example of comparing yellow and red candies, we could have drawn a sample of yellow candies first and as a next step draw a sample of red candies. The red candies sample is independent from the yellow candies sample because yellow candies are completely irrelevant to sampling red candies. Even if we draw red and yellow candies during one sampling procedure, the yellow candies doe not affect the sample of red candies and the other way around. It is the principle that counts: samples are independent if we could have drawn the separately. 

#### Theoretical approximation in SPSS

By default, SPSS uses a theoretical probability distribution to approximate the sampling distribution. It chooses the correct theoretical distribution but you yourself should check if the conditions are met. Is the sample size large enough or is it plausible that the variable is normally distributed in the population?

In one case, SPSS automatically selects an exact aproach if the conditions for a theoretical approximation is not met. If you tell SPSS to aply a chi-squared test to a cross tabulation, SPSS will automatically apply Fisher's exact test if the table has two rows and two columns. In all other cases, you have to select a bootstrapping or exact aproach yourself if the conditions for a theoretical approximaiton are not met.  
...
```{r point_estimates}
# A button allows to draw a sample of size 20 from a population with five colours and random proportion of yellow candies within the range .10 .40 ; the sample is shown as a dotplot (stacks of coloured dots) with the proportion of yellow as a number ; result: true population proportion, sample proportion, and student's proportion with current winner (closest to true population proportion) and a bar chart showing cumulative number of wins for sample proportion versus student  sum of mistakes over all samples
```

\@ref(fig:point_estimates) generates random samples from a population of candies for which we do not know the proportion of yellow candies. It uses the sample proportion as estimate of the population proportion. Note that each sample is drawn from a different population. 

1. Can you do better? Draw a sample and guess the population proportion. See which estimate is better. Repeat this until you are convinced that you can do better or cannot do better than the sample proportion.  

If we have to name one value for the population value, our best guess is the value of the sample statistic. For example, if 18% of the candies in our sample bag are yellow, our best guess of the proportion of yellow candies in the population of all candies from which this bag was filled, is .18. What other number can we give if we only have our sample? This type of guess is called a _point estimate_ and it is used a lot.  
...
The sampling distribution tells us the probabilities of finding particular sample statistic values in the case of a discrete random variable, for example, the number of yellow candies in a sample bag. It tells us the probability of finding a range of scores in the case of a continuous random variable such as the average weight of candies in a sample bag.  

Therefore, the sampling distribution allows us to calculate the probability of a sample with a particular (minimum or maximum) value for the sample statistic. We can also use it to calculate the probability of finding a sample statistic between two values, that is, within a range of values. We use this range in our interval estimate.  

The average or expected value of a sampling distribution, we have learned before, is equal to the population value if the estimator is unbiased. For example, the mean weight of yellow candies averaged over very many samples is equal to the mean weight of yellow candies in the population. For an interval estimate, we now select the sample statistic values that are closest to the population value, hence, to the mean of the sampling distribution. Between which borders are the sample statistic values situated that are closest to the population value?  

To find the borders, we must choose the proportion of sample statistic values that we are interested in. Which part of all samples do we want to include? A popular proportion is 95%, so we want to know the border values that include 95% of all samples that are closest to the population value. 

```{r ci_borders}
# Graph a normal distribution with mean 2.8 and standard deviation a random number between 0.05 and 0.2 ; x-axis labelled "Average candy weight"; add two vertical lines, one to the extreme left, one to the extreme right with their values on the x-axis displayed and the percentage of observations (area) between the two sliders displayed (initially near 100%) ; slider moves right line and left line (in opposite directions) and adjusts percentage of area between lines ; note that the lines cannot be moved across the center of the distribution
```

\@ref(fig:ci_borders) shows the sampling distribution of average sample candy weight.
...
We now have border values, that is, a range of sample statistic values, and a probability of drawing a sample with a statistic falling within this range. The probability gives us some confidence in the estimate. We may be wrong to conclude that average candy weight in the population is between 2.7 and 2.9 gram. But if we are wrong, our sample is very improbable, having less than 5 percent a probability to occur.   

### 3.3 Precision, Standard Error, and Sample Size

The width of the estimated interval represents the _precision_ of our estimate. The wider the interval, the less precise our estimate. With a less precise interval estimate, we reckon with a wider variety of outcomes in our sample. If we want to predict something, however, we value precision. We rather conclude that the average weight of candies in the next sample we draw lies between 2.75 and 2.85 gram than between 1.5 and 4.1 gram. If we would be satisfied by a very imprecise estimate, we need not do any research at all. With a little knowledge about the candies that we are investigating, we could straightaway predict that the average candy weight is between zero and ten gram. The goal of our research is to make more precise estimations.  
...
In the interactive content (above), we approximate the sampling distribution with a theoretical probability distribution, namely the normal distribution. If we use a theoretical probability distribution, the selected probability level corresponds to a particular outcome value in the theoretical probability distribution.  
...
The interactive content below shows a lot of standard normal distributions that we could use to approximate the sampling distribution of the average candy weight in a sample. Of course, we can only use one. How can you pick the one that we need?  
...
A test for this type of statistical hypothesis is called a one-sample chi-squared test {_check_}. It is up to the researcher to specify the hypothesized proportions for all categories. This is not a simple task: What reasons do you have to expect particular values, say a reach of fifty percent of all households instead of sixty percent?

The test is mainly used if the researcher has information on the proportions of the categories in the population. If we draw a sample from the citizens of a country, for example, we usually know from the national bureau of statistics the frequency distribution of sex, age, educational level, and so on of all citizens. We can now test if the respondents in our sample have the same distribution with respect to sex, age, or educational level as the population; just use the population proportions in the hypothesis. If they do, the sample is _representative_ of the population with respect to sex, age, or educational level. This is an important check on the representativeness of our sample.
...
Finally, research hypotheses may address the relation between two or more variables. Relations between variables are at stake if the research hypothesis states or implies that one (type of) characteristic is related to another (type of) characteristic. For example, if the identity of the endorser makes a difference to the willingness to donate, the endorser to whom a person is exposed (one characteristic) is related this person's willingness to donate. Another example: If exposure to brand advertisements increases brand awareness, a person's brand awareness is positively related to this person's exposure to advertisements for the brand.  
...
In this case, it makes sense to care only for the right tail of the sampling distribution. The entire probability of rejecting the null hypothesis while it is actually true, that is, the significance level, is located in the right tail of the sampling distribution. Of course, a one-sided test may also focus solely on the left tail of the sampling distribution, for example, if we hypothesize that average media literacy of children is below 5.5.  
...
In the first step, translate the research hypothesis into a null and alternative hypothesis. This requires choosing the right statistics for testing the research hypothesis and hosing between a one-sided or two-sided test if applicable.  
...
The interpretation of this test on a proportion tells us the population ("all households in Greece"), the variable ("reaching a household") and sample statistic of interest (_p_ for proportion). It tells us that the result is statistically significant, which a fellow scientist can check with the reported p-value. Note that the actual p-value is well below .001, so if we would round it to three decimals, it would become .000. The latter notation suggests that the probability is zero but there is always some probability of rejecting the null hypothesis if it is true. For this reason, APA6 wants us to report p < .001 instead of p = .000. Finally, the interpretation tells us that the difference from .5 is substantial. Sometimes, we can express the difference in a number, which is called the _effect size_, and give a more precise interpretation. See Chapter 5 for more information.  
...
The interactive content below shows a sampling distribution of sample medians for media literacy of teenagers. Determine a confidence interval and test the null hypothesis that teenager media literacy in the population is 6.0.  
...
Conditions often include sample size. If you plan to do a _t_-test, either on its own or in post-hoc tests after analysis of variance, each group should contain more than thirty observations. So if you plan on doing _t_-tests, recruit more than thirty subjects for your experiment or more than thirty respondents for your survey and you are fine. Well, if you have to reckon with non-response, that is, subjects or respondents unwilling to participate in your research, you should address as many subjects or respondents necessary to have more than thirty observations in the end.  
...
The direction of an effect is not relevant to effect size. For example, we do not care whether the yellow candies are on average heavier or the red candies. For this reason, Cohen's _d_ is always positive. If you obtain a negative result, just drop the minus sign.   
...
Well, try to test this null hypothesis in your favourite statistics software. Can you actually tell the software that the null hypothesis about this regression coefficient is 0.1? Most likely, you can't because the software automatically tests the null hypothesis that the regression coefficient is zero in the population. This approach is so prevalent that null hypotheses equating the population value of interest to zero have received a special name: the _nill hypothesis_ or the _nill_ for short. So you simply cannot include previous knowledge in your test here.  
...
Rejecting the nil time and again should make us wonder about scientific progress and our contribution to it. Is there no way to accumulate our efforts?  
...
The problems with null hypothesis significance testing, however, are increasingly being recognized. Alternatives to null hypothesis significance testing have been developed and are becoming more accepted within the field. In this section, some alternatives are briefly sketched.  
...
Hayes () on process (model) inference (Section @) and on mediation and moderation
